...
object TestDataset {
  ...
  private val ergoBlock2 = parse(readJsonFile("./test/dataset/SampleStealth_ergoFullBlock2.json")).toOption.get.as[ErgoFullBlock].toOption.get

  private val headerJson: String = readJsonFile("./test/dataset/SampleStealth_ergoFullBlockHeader1.json")
  ...
  def newBlockJson: (String, String, String) = (headerJson, headerIdJson, blockJson)

  private val fakeStealthScript: String =
    s"""{
  ...
  private val stealthScript: String =
    s"""{
       |  val gr = decodePoint(fromBase64("GR"))
       |  val gy = decodePoint(fromBase64("GY"))
       |  val ur = decodePoint(fromBase64("UR"))
  ...
  def stealthScripts : (String, String) = (fakeStealthScript, stealthScript)
}
