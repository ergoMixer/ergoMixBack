...
object ErgoMixerDataset extends DatasetSuite {
  ...
  def jsonToMixingBoxPrices(jsonString: String): Seq[(Long, Long)] =
    parser.parse(jsonString) match {
      ...
      case Right(js) =>
        js.hcursor.downField("items").as[Seq[Json]] match {
          case Left(e) => throw new Exception(s"Error while parsing MixingBox from Json: $e")
          case Right(arr) =>
            arr.map { js =>
  ...
  private val sample0_MixingRequest: MixingRequest = CreateMixingRequest(
    readJsonFile("./test/dataset/Sample0_MixRequest.json")
  )
  private val sample0_MixRequest: MixRequest = sample0_MixingRequest.toMixRequest
  private val sample0_MixCovertRequest: MixCovertRequest = CreateMixCovertRequest(
    readJsonFile("./test/dataset/Sample0_MixCovertRequest.json")
  )
  private val sample0_CovertAsset: CovertAsset = CreateCovertAsset(
    readJsonFile("./test/dataset/Sample0_CovertAsset.json")
  )
  ...
  private val sample0_HalfMix: Option[HalfMix] = Option(
    CreateHalfMix(readJsonFile("./test/dataset/Sample0_HalfMix.json"))
  )
  ...
  private val sample2_masterKey_2: BigInt = BigInt(
    "66760146388183827226642846566692984587383552135637379597027429969154056748902"
  )

  private val sample3_MixingRequest: MixingRequest = CreateMixingRequest(
    readJsonFile("./test/dataset/Sample3_MixRequest.json")
  )
  ...
  private val sample4_MixingRequest: MixingRequest = CreateMixingRequest(
    readJsonFile("./test/dataset/Sample4_MixRequest.json")
  )
  ...
  private val sampleArray_MixGroupRequestPrices = jsonToMixingBoxPrices(
    readJsonFile("./test/dataset/SampleNewGroup_MixingBoxPrices.json")
  )
  private val sampleArray_MixGroupRequestAll = CreateMixGroupRequest(
    readJsonFile("./test/dataset/Sample0_MixGroupRequest.json")
  )

  private val sample6_MixingRequest: MixingRequest = CreateMixingRequest(
    readJsonFile("./test/dataset/Sample6_MixRequest.json")
  )
  ...
  private val sample6_Withdraw = Option(
    CreateWithdraw(
      Array(
        sample6_WithdrawTx.get.mixId,
        sample6_WithdrawTx.get.txId,
        sample6_WithdrawTx.get.time,
        sample6_WithdrawTx.get.boxId,
        sample6_WithdrawTx.get.txBytes
      )
    )
  )

  private val sample7_MixingRequest: MixingRequest = CreateMixingRequest(
    readJsonFile("./test/dataset/Sample7_MixRequest.json")
  )
  ...
  private val sample7_Withdraw = Option(
    CreateWithdraw(
      Array(
        sample7_WithdrawTx.get.mixId,
        sample7_WithdrawTx.get.txId,
        sample7_WithdrawTx.get.time,
        sample7_WithdrawTx.get.boxId,
        sample7_WithdrawTx.get.txBytes
      )
    )
  )

  private val sample_AllMixingRequests = Seq(
    sample0_MixingRequest,
    sample3_MixingRequest,
    sample4_MixingRequest,
    sample6_MixingRequest,
    sample7_MixingRequest
  )

  private val sample_AllMixStates =
    Seq(sample0_MixState, sample3_MixState, sample4_MixState, sample6_MixState, sample7_MixState)
  ...
  private val sample0_Mix =
    Mix(sample0_MixRequest, Option(sample0_MixState), sample0_HalfMix, sample0_FullMix, sample0_Withdraw)
  private val sample3_Mix =
    Mix(sample3_MixRequest, Option(sample3_MixState), sample3_HalfMix, sample3_FullMix, sample3_Withdraw)
  private val sample4_Mix =
    Mix(sample4_MixRequest, Option(sample4_MixState), sample4_HalfMix, sample4_FullMix, sample4_Withdraw)
  private val sample6_Mix =
    Mix(sample6_MixRequest, Option(sample6_MixState), sample6_HalfMix, sample6_FullMix, sample6_Withdraw)
  private val sample7_Mix =
    Mix(sample7_MixRequest, Option(sample7_MixState), sample7_HalfMix, sample7_FullMix, sample7_Withdraw)
  ...
  private val emptyAddress_MixingRequest: MixingRequest = CreateMixingRequest(
    readJsonFile("./test/dataset/SampleEmptyAddress_MixRequest.json")
  )
  private val emptyAddress_MixId = emptyAddress_MixingRequest.id

  private val sample12_MixingRequest: MixingRequest = CreateMixingRequest(
    readJsonFile("./test/dataset/Sample12_MixRequest.json")
  )
  private val sample12_MixId: String = sample12_MixingRequest.id

  private val sample16_hopMixList =
    jsonToObjectList[HopMix](readJsonFile("./test/dataset/Sample16_HopMixList.json"), CreateHopMix.apply)
   ...
   */
  def existsCovertAsset: (String, String, Long, MixCovertRequest, CovertAsset) = (
    sample0_CovertAsset.covertId,
    sample0_CovertAsset.tokenId,
    200000000000L,
    sample0_MixCovertRequest,
    sample0_CovertAsset
  )

  def notExistsCovertAsset: (String, String, Long, MixCovertRequest) =
    (sample0_CovertAsset.covertId, sample0_CovertAsset.tokenId, 200000000000L, sample0_MixCovertRequest)
   ...
   */
  def withdrawAddressOfMixRequest: (String, String, MixingRequest) =
    (sample0_MixId, sample0_WithdrawAddress, sample0_MixingRequest)
   ...
   * @return MixState mixState
   */
  def notWithdrawnMixingRequests: (String, Map[String, Long], Seq[MixingRequest]) =
    (sample0_GroupId, sampleMap_All, sample_SeqMixingRequests)
   ...
   * @return MixState mixState
   */
  def notWithdrawnRunningMixRequests: (String, Map[String, Long], Seq[MixingRequest]) =
    (sample0_GroupId, sampleMap_Running, sample_SeqRunningMixingRequests)
   ...
   * all spec data
   */
  def newGroupData: (mutable.Iterable[MixingBox], Seq[(Long, Long)], MixGroupRequest) =
    (sampleArray_MixGroupRequest, sampleArray_MixGroupRequestPrices, sampleArray_MixGroupRequestAll)
   ...
   * @return Int countAll
   */
  def roundFinishedData: (String, Int, Int, Int, Seq[MixingRequest]) =
    (sample0_GroupId, 2, 3, 5, sample_AllMixingRequests)
   ...
   */
  def roundProgressData: (String, (Int, Int), Seq[MixingRequest], Seq[MixState]) =
    (sample0_GroupId, (60, 10), sample_AllMixingRequests, sample_AllMixStates)
   ...
   */
  def dbMixes: (Seq[MixingRequest], Seq[MixState], Seq[WithdrawTx], Seq[HalfMix], Seq[FullMix]) =
    (sample_AllMixingRequests, sample_AllMixStates, sample_AllMixWithdraws, sample_HalfMixes, sample_FullMixes)
