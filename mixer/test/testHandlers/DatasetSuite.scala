...
class DatasetSuite {
  ...
  def jsonToSpentTx(jsonString: String): SpendTx = {
    val json =
      parse(jsonString).getOrElse(throw new Exception("Error while converting SpendTx jsonString to Json object"))
  ...
  def jsonToSignedTransaction(jsonString: String): SignedTransaction = {
    val json = parse(jsonString).getOrElse(
      throw new Exception("Error while converting SignedTransaction jsonString to Json object")
    )
    val id                    = (json \\ "id").map(v => v.asString.get).head
