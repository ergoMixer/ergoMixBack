...
object HopMixerDataset extends DatasetSuite {
  ...
  private val sample13_SignedTransaction = jsonToSignedTransaction(
    readJsonFile("./test/dataset/Sample13_SignedTransaction.json")
  )
  ...
  def withdrawFromHop_ergoMixerMockedData: (String, BigInt, String) =
    (sample12_HopMix.mixId, sample12_MixingRequest.masterKey, sample12_MixingRequest.withdrawAddress)

  def withdrawFromHop_aliceOrBobMockedData: (BigInteger, String, String, SignedTransaction) =
    (sample12_Secret, sample12_HopMix.boxId, sample12_MixingRequest.withdrawAddress, sample12_SignedTransaction)
  ...
  def nextHop_aliceOrBobMockedData: (BigInteger, String, String, SignedTransaction) =
    (sample13_Secret, sample13_HopMix.boxId, sample13_NextHopAddress, sample13_SignedTransaction)
