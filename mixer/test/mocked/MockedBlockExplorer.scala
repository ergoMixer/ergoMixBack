...
class MockedBlockExplorer extends MockitoSugar {
   ...
   */
  def setReturnValue_getTxNumConfirmations(txId: String, numConfirm: Int): Unit =
    when(blockExplorer.getTxNumConfirmations(txId)).thenReturn(numConfirm)
   ...
   */
  def setReturnValue_getConfirmationsForBoxId(boxId: String, numConfirm: Int): Unit =
    when(blockExplorer.getConfirmationsForBoxId(boxId)).thenReturn(numConfirm)
   ...
   */
  def setReturnValue_getSpendingTxId(boxId: String, txId: String): Unit =
    when(blockExplorer.getSpendingTxId(boxId)).thenReturn(Option(txId))
   ...
   */
  def setReturnValue_getTransaction(txId: String, tx: SpendTx): Unit =
    when(blockExplorer.getTransaction(txId)).thenReturn(Option(tx))
   ...
   */
  def setReturnValue_getBoxIdsByAddress(address: String, boxIds: Seq[String]): Unit =
    when(blockExplorer.getBoxIdsByAddress(address)).thenReturn(boxIds)
