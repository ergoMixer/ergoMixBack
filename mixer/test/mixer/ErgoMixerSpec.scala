...
class ErgoMixerSpec extends TestSuite {
  ...
  property("newCovertRequest should return valid address when privateKey is empty") {
    ...
    daoUtils.awaitResult(daoContext.mixingCovertRequestDAO.clear)
    daoUtils.awaitResult(daoContext.covertAddressesDAO.clear)
    daoUtils.awaitResult(daoContext.covertDefaultsDAO.clear)
    ...
    val db_reqs: Seq[(String, Int, Boolean)] = daoUtils
      .awaitResult(daoContext.mixingCovertRequestDAO.all)
      .map(req => (req.nameCovert, req.numRounds, req.isManualCovert))
  ...
  property("newCovertRequest should return valid address when privateKey does not exists in database") {
    ...
    // verify insertion of data in database
    val db_reqs: Seq[(String, Int, Boolean, BigInt)] = daoUtils
      .awaitResult(daoContext.mixingCovertRequestDAO.all)
      .map(req => (req.nameCovert, req.numRounds, req.isManualCovert, req.masterKey))
  ...
  property("newCovertRequest should throw exception when privateKey exists in database") {
    ...
    // make dependency tables ready before test
    daoUtils.awaitResult(daoContext.mixingCovertRequestDAO.clear)
    daoUtils.awaitResult(daoContext.mixingCovertRequestDAO.insert(testSample_masterKeyData._2))
  ...
  property("handleCovertSupport should update ring when asset exists") {
    ...
    daoUtils.awaitResult(daoContext.mixingCovertRequestDAO.clear)
    daoUtils.awaitResult(daoContext.covertDefaultsDAO.clear)
    daoUtils.awaitResult(daoContext.mixingCovertRequestDAO.insert(existsCovert))
    daoUtils.awaitResult(daoContext.covertDefaultsDAO.insert(existsAsset))

    val ergoMixer = createErgoMixerObject
    ergoMixer.handleCovertSupport(covertId, tokenId, ring)

    // verify the update in database
    val db_assets: Seq[(String, String, Long)] =
      daoUtils.awaitResult(daoContext.covertDefaultsDAO.all).map(asset => (asset.covertId, asset.tokenId, asset.ring))
  ...
  property("getWithdrawAddress should return string when mixId exists in database") {
    ...
    daoUtils.awaitResult(daoContext.mixingRequestsDAO.insert(request))
  ...
  property("getRoundNum should return int when mixId exists in database") {
    ...
    daoUtils.awaitResult(daoContext.mixStateDAO.insert(state))
   ...
   */
  property(
    "withdrawMixNow should throw exception if withdrawAddress is empty or call updateWithdrawStatus if it exists"
  ) {
    // get test data from dataset
    ...
    try {
      ...
      case e: Exception =>
        if (e.getMessage != "Set a valid withdraw address first!") fail(s"Wrong exception: ${e.getMessage}")
    }
    ...
    // verify the update in database
    val db_reqs: Seq[(String, String)] =
      daoUtils.awaitResult(daoContext.mixingRequestsDAO.all).map(req => (req.id, req.withdrawStatus))
  ...
  property("getCovertCurrentMixing should return Map[String, Long] when covertId exists in database") {
    ...
    // make dependency tables ready before test
    daoUtils.awaitResult(daoContext.mixingRequestsDAO.clear)
    reqs.foreach(req => daoUtils.awaitResult(daoContext.mixingRequestsDAO.insert(req)))
  ...
  property("newMixRequest should return valid address") {
    ...
    val ergoMixer = createErgoMixerObject
    val address = ergoMixer.newMixRequest(
      testSample._1,
      testSample._2,
      testSample._3,
      testSample._4,
    ...
    // verify insertion of data in database
    val db_reqs: Seq[(String, Int, Long, Long, Long, Long, String, String)] = daoUtils
      .awaitResult(daoContext.mixingRequestsDAO.all)
      .map(req =>
        (
          req.withdrawAddress,
          req.numRounds,
  ...
  property("newMixGroupRequest should return string without any error") {
    ...
    // make dependency tables ready before test
    daoUtils.awaitResult(daoContext.mixingRequestsDAO.clear)
    daoUtils.awaitResult(daoContext.mixingGroupRequestDAO.clear)

    val ergoMixer     = createErgoMixerObject
    val mixId: String = ergoMixer.newMixGroupRequest(requestArray)

    // verify insertion of data in database
    val db_reqs: Seq[(String, Int, Long, Long, Long, Long, String)] = daoUtils
      .awaitResult(daoContext.mixingRequestsDAO.all)
      .map(req =>
        (
          req.withdrawAddress,
    ...
    requestArray.zip(requestPrices).foreach {
      case (req, price) =>
        db_reqs should contain(
          (
            req.withdraw,
    ...
    val db_groupReqs: Seq[(Long, String, Long, Long, Long, Long, Long, String)] = daoUtils
      .awaitResult(daoContext.mixingGroupRequestDAO.all)
      .map(req =>
        (
          req.neededAmount,
          req.status,
  ...
  property("getProgressForGroup should return (Long, Long)") {
    ...
    daoUtils.awaitResult(daoContext.mixStateDAO.clear)
    reqs.foreach(req => daoUtils.awaitResult(daoContext.mixingRequestsDAO.insert(req)))
    states.foreach(state => daoUtils.awaitResult(daoContext.mixStateDAO.insert(state)))
  ...
  property("getMixes should return Seq[Mixes]") {
    ...
    reqs.foreach(req => daoUtils.awaitResult(daoContext.mixingRequestsDAO.insert(req)))
    states.foreach(state => daoUtils.awaitResult(daoContext.mixStateDAO.insert(state)))
    withdraws.foreach(mix => daoUtils.awaitResult(daoContext.withdrawDAO.insert(mix)))
    halves.foreach(mix => daoUtils.awaitResult(daoContext.halfMixDAO.insert(mix)))
    fulls.foreach(mix => daoUtils.awaitResult(daoContext.fullMixDAO.insert(mix)))
