...
class RescanSpec
   ...
   *    )
   */
  property("processRescan should insert recovered rounds of mix, hop and withdraw into their corresponding tables (rescan fullBox)") {
    ...
    // verify return value
    val db_mixingRequest = daoUtils.awaitResult(daoContext.mixingRequestsDAO.all).map(req => (req.id, req.mixStatus, req.withdrawStatus))
    db_mixingRequest should contain((testSample_mixingRequest.id, Complete, UnderHop.value))
    val db_fullMix = daoUtils.awaitResult(daoContext.fullMixDAO.all).map(mix => (mix.mixId, mix.round, mix.halfMixBoxId, mix.fullMixBoxId))
    db_fullMix should equal (fullMixList)
    val db_halfMix = daoUtils.awaitResult(daoContext.halfMixDAO.all).map(mix => (mix.mixId, mix.round, mix.halfMixBoxId))
    db_halfMix should equal (halfMixList)
    val db_mixStateHistory = daoUtils.awaitResult(daoContext.mixStateHistoryDAO.all).map(mix => (mix.id, mix.round, mix.isAlice))
    db_mixStateHistory should equal (mixStateHistoryList)
    val db_mixState = daoUtils.awaitResult(daoContext.mixStateDAO.all).map(state => (state.id, state.round, state.isAlice))
   ...
   *    )
   */
  property("processRescan should insert recovered rounds of hop and withdraw into their corresponding tables (rescan hopBox)") {
    // get test data from dataset
