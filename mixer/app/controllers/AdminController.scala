...
class AdminController @Inject() (
  ...
  def setIncomeParams(): Action[AnyContent] = Action { implicit request: Request[AnyContent] =>
    try {
      ...
      val commissionFee =
        js.hcursor.downField("commissionFee").as[Int].getOrElse(throw new Exception("commissionFee param is needed"))
      val levels: Seq[(Int, Long)] = js.hcursor
        .downField("levels")
        .as[Seq[Json]]
        .getOrElse(throw new Exception("levels param is needed"))
        .map { level =>
          (
  ...
  def getIncomeParams: Action[AnyContent] = Action { implicit request: Request[AnyContent] =>
    if (AdminConfigs.tokenLevels.isEmpty) {
      BadRequest(BAD_REQUEST_IF_DATA_DOES_NOT_LOAD).as("application/json")
    } else {
  ...
  def setFeeParams(): Action[AnyContent] = Action { implicit request: Request[AnyContent] =>
    try {
      ...
      val dynamicFee =
        js.hcursor.downField("dynamicFee").as[Long].getOrElse(throw new Exception("dynamicFee param is needed"))
  ...
  def addSupport(): Action[AnyContent] = Action { implicit request: Request[AnyContent] =>
    try {
      ...
      AdminConfigs.toAddSupport =
        AdminConfigs.toAddSupport :+ (new EntityInfo(name, id, rings, decimals, AdminConfigs.desiredFee._2), null)
      Ok(s"""{"success": true}""".stripMargin).as("application/json")
  ...
  def removeSupport(): Action[AnyContent] = Action { implicit request: Request[AnyContent] =>
    try {
      ...
      AdminConfigs.toRemoveSupport =
        AdminConfigs.toRemoveSupport :+ new EntityInfo(name, id, rings, decimals, AdminConfigs.desiredFee._2)
      Ok(s"""{"success": true}""".stripMargin).as("application/json")
