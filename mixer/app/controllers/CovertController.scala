 ...
 */
class CovertController @Inject() (
  controllerComponents: ControllerComponents,
  ergoMixerUtils: ErgoMixerUtils,
  ergoMixer: ErgoMixer,
  covertMixer: CovertMixer
  ...
  def covertList: Action[AnyContent] = Action {
    try {
      val coverts = ergoMixer.getCovertList
        .map { covert =>
  ...
  def withdrawCovertAsset(covertId: String): Action[AnyContent] = Action { implicit request: Request[AnyContent] =>
    try {
      val js = io.circe.parser.parse(request.body.asJson.get.toString()).getOrElse(Json.Null)
      val tokenIds: Seq[String] =
        js.hcursor.downField("tokenIds").as[Seq[String]].getOrElse(throw new Exception(s"tokenIds is required"))
      val withdrawAddress: String = js.hcursor
        .downField("withdrawAddress")
        .as[String]
        .getOrElse(throw new Exception(s"withdrawAddress is required"))
