...
class BobImpl(y: BigInteger, implicit val tokenErgoMix: TokenErgoMix)(implicit ctx: BlockchainContext) extends Bob {
  ...
  def spendFullMixBox(
    ...
    val bob: ErgoProver = additionalDHTuples
      .foldLeft(
  ...
  def spendHalfMixBox(
    ...
    val firstOutBox = txB
      ...
      .registers(
        ErgoValue.of(c1),
        ErgoValue.of(c2),
        ErgoValue.of(halfMixBox.gX),
        ErgoValue.of(tokenErgoMix.halfMixScriptHash)
      )
      .contract(tokenErgoMix.fullMixScriptContract)
      .tokens(tokens: _*)
      .build()

    val secondOutBox = txB
      .outBoxBuilder()
      .value(halfMixBox.inputBox.getValue)
      .registers(
        ErgoValue.of(c2),
    ...
    if (tokenBox.getRegisters.size() == 2) {
      ...
      if (mixingToken != null) {
        val inputTokens =
          inputBoxes.map(_.getTokens.asScala.filter(_.getId == mixingToken.getId).map(_.getValue).sum).sum
    ...
    val preTxB = txB
      ...
      .sendChangeTo(Address.create(changeAddress))
    val txToSign =
      if (distrBurnToken > 0) preTxB.tokensToBurn(new ErgoToken(TokenErgoMix.tokenId, distrBurnToken)).build()
      else preTxB.build()
