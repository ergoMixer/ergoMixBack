...
class AliceOrBob @Inject() (implicit val networkUtils: NetworkUtils, explorer: BlockExplorer) {
  ...
  def spendBox(
    inputBox: String,
    feeBox: Option[String],
    withdrawAddress: String,
    proverDlogSecrets: Array[BigInteger],
    feeAmount: Long,
    broadCast: Boolean
  ): SignedTransaction =
    usingClient { implicit ctx =>
      val prover: ErgoProver = proverDlogSecrets
        .foldLeft(ctx.newProverBuilder())((proverBuilder, bigInteger) => proverBuilder.withDLogSecret(bigInteger))
        .build()
  ...
  def distribute(
    ...
    usingClient { implicit ctx =>
      ...
      for (i <- 0 until numTxs) {
        ...
        if (remErg > 0) {
          val tokens = inTokens
            .filter(tok => tok._2 - outTokens.getOrElse(tok._1, 0L) > 0)
  ...
  def spendFullMixBox(
    ...
    usingClient { implicit ctx =>
      ...
      val distrBurnToken = new ErgoToken(TokenErgoMix.tokenId, distrBurnTokenAmount)
      val endBox = EndBox(
        WalletHelper.getErgoAddress(withdrawAddress).script,
        Nil,
        if (inputBoxIds.nonEmpty) fullMixBox.getValue else fullMixBox.getValue - feeAmount,
        tokens
      )
      var outs = Seq(endBox)
      if (inputBoxIds.length > 0) { // there is fee box
        ...
        val feeCp =
          EndBox(ergoMix.feeEmissionContract.getErgoTree, feeBox.getRegisters.asScala, feeBox.getValue - feeAmount)
        outs = outs :+ feeCp
      }
      val tx: SignedTransaction = alice_or_bob.spendFullMixBox(
        FullMixBox(fullMixBox),
        outs,
        feeAmount,
        inputBoxIds,
        changeAddress,
   ...
   */
  def spendFullMixBox_RemixAsAlice(
    isAlice: Boolean,
    secret: BigInt,
    fullMixBoxId: String,
    nextSecret: BigInt,
    feeEmissionBoxId: String,
    feeAmount: Long
  ): HalfMixTx =
    usingClient { implicit ctx =>
      val feeEmissionBox        = ctx.getBoxesById(feeEmissionBoxId)(0)
      val feeEmissionBoxAddress = WalletHelper.getErgoAddress(feeEmissionBox.getErgoTree).toString
      spendFullMixBox_RemixAsAlice(
        isAlice,
        secret,
        fullMixBoxId,
        nextSecret,
        Array(feeEmissionBoxId),
        feeAmount,
        feeEmissionBoxAddress,
        Seq(),
        broadCast = false
      )
    }
   ...
   */
  private def spendFullMixBox_RemixAsAlice(
    isAlice: Boolean,
    secret: BigInt,
    fullMixBoxId: String,
    nextSecret: BigInt,
    ...
    usingClient { implicit ctx =>
      val alice_or_bob         = getProver(secret, isAlice)
      val fullMixBox: InputBox = ctx.getBoxesById(fullMixBoxId)(0)
      val halfMixTx = alice_or_bob.spendFullMixBoxNextAlice(
        FullMixBox(fullMixBox),
        nextSecret.bigInteger,
        feeAmount,
        inputBoxIds,
        changeAddress,
        changeBoxRegs,
   ...
   */
  def spendFullMixBox_RemixAsBob(
    isAlice: Boolean,
    secret: BigInt,
    fullMixBoxId: String,
    nextSecret: BigInt,
   ...
   */
  private def spendFullMixBox_RemixAsBob(
    isAlice: Boolean,
    secret: BigInt,
    fullMixBoxId: String,
    nextSecret: BigInt,
    nextHalfMixBoxId: String,
    inputBoxIds: Array[String],
    feeAmount: Long,
    changeAddress: String,
    changeBoxRegs: Seq[ErgoValue[_]],
    ...
    usingClient { implicit ctx =>
      val alice_or_bob: FullMixBoxSpender =
      ...
      val (fullMixTx, bit) = alice_or_bob.spendFullMixBoxNextBob(
        FullMixBox(fullMixBox),
  ...
  def spendHalfMixBox(
    ...
    feeAmount: Long,
    changeAddress: String,
    proverDlogSecrets: Array[String],
    broadCast: Boolean,
    numToken: Int = 0
  ): (FullMixTx, Boolean) =
    usingClient { implicit ctx =>
      ...
      val (fullMixTx, bit) = bob.spendHalfMixBox(
        HalfMixBox(halfMixBox),
        inputBoxIds,
        feeAmount,
        changeAddress,
   ...
   */
  def getProver(secret: BigInt, isAlice: Boolean)(
    implicit ctx: BlockchainContext
  ): mixinterface.ErgoMixBase.FullMixBoxSpender =
    if (isAlice) new AliceImpl(secret.bigInteger, tokenErgoMix.get)
    else new BobImpl(secret.bigInteger, tokenErgoMix.get)
  ...
  def spendHopBox(proverDlogSecret: BigInteger, inputId: String, destinationAddress: String): SignedTransaction =
    usingClient { implicit ctx =>
      ...
      if (!input.getTokens.isEmpty)
        throw new Exception("Weird case happened: hopBox containing token. Aborting hopping transaction generation...")
      inputs.add(input)
  ...
  def withdrawToken(
    ...
    usingClient { implicit ctx =>
      val prover: ErgoProver = Array(proverDlogSecrets)
        .foldLeft(ctx.newProverBuilder())((proverBuilder, bigInteger) => proverBuilder.withDLogSecret(bigInteger))
        .build()
      ...
      if (inputErgValues < MainConfigs.distributeFee + outBoxCount * MainConfigs.minPossibleErgInBox) {
        ...
        allBoxes.foreach { box =>
          ...
          if (
            inputErgValues < MainConfigs.distributeFee + outBoxCount * MainConfigs.minPossibleErgInBox && !inputIds
              .contains(box.id)
          ) {
        ...
        if (inputErgValues < MainConfigs.distributeFee + outBoxCount * MainConfigs.minPossibleErgInBox)
          throw new Exception(
            s"Not enough erg. current value: $inputErgValues, required: ${MainConfigs.distributeFee + outBoxCount * MainConfigs.minPossibleErgInBox}"
          )
      }
      val txC = ctx.newTxBuilder()
