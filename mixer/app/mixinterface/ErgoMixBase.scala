...
package object ErgoMixBase {
  ...
  trait FullMixBoxSpender {
    def spendFullMixBox(
      ...
      spendFullMixBox(
        ...
        endBoxes,
        feeAmount,
        ctx.getBoxesById(otherInputBoxIds: _*),
        changeAddress,
        changeBoxRegs,
        burnTokens,
        additionalDlogSecrets,
        additionalDHTuples
      )
    ...
    def spendFullMixBoxNextAlice(
      ...
      additionalDlogSecrets: Array[BigInteger],
      additionalDHTuples: Array[DHT],
      dynamicFee: Boolean = true
    )(implicit ctx: BlockchainContext, networkUtils: NetworkUtils): HalfMixTx = {
      ...
      val endBox = EndBox(
        ergoMix.halfMixContract.getErgoTree,
        Seq(ErgoValue.of(WalletHelper.g.exp(nextX))),
        f.inputBox.getValue,
        tokens
      )
      val feeCp = EndBox(
        ergoMix.feeEmissionContract.getErgoTree,
        otherInputBoxes(0).getRegisters.asScala,
        otherInputBoxes(0).getValue - feeAmount
      )
      val signedTransaction = spendFullMixBox(
        f,
        Seq(endBox, feeCp),
        feeAmount,
        otherInputBoxes,
        changeAddress,
        changeBoxRegs,
        Seq(fullBoxBurnToken),
        additionalDlogSecrets,
        additionalDHTuples
      )
      val expectedFee = ErgoLikeTransaction.serializer
        .toBytes(signedTransaction.asInstanceOf[SignedTransactionImpl].getTx)
        .length * MainConfigs.dynamicFeeRate
      if (expectedFee != feeAmount && dynamicFee) {
        return spendFullMixBoxNextAlice(
          f,
          nextX,
          expectedFee,
          otherInputBoxes,
    ...
    def spendFullMixBoxNextBob(
      ...
      additionalDlogSecrets: Array[BigInteger],
      additionalDHTuples: Array[DHT]
    )(implicit ctx: BlockchainContext, networkUtils: NetworkUtils): (FullMixTx, Boolean) =
      spendFullMixBoxNextBob(
        f,
        h,
        nextY,
        feeAmount,
    ...
    def spendFullMixBoxNextBob(
      ...
      val outBox1 = EndBox(
        ergoMix.fullMixScriptErgoTree,
        Seq(ErgoValue.of(c1), ErgoValue.of(c2), ErgoValue.of(gX), ErgoValue.of(ergoMix.halfMixScriptHash)),
        f.inputBox.getValue,
        tokens
      )
      val outBox2 = EndBox(
        ergoMix.fullMixScriptErgoTree,
        Seq(ErgoValue.of(c2), ErgoValue.of(c1), ErgoValue.of(gX), ErgoValue.of(ergoMix.halfMixScriptHash)),
        f.inputBox.getValue,
        tokens
      )
      val feeCp = EndBox(
      ...
      val tx: SignedTransaction = spendFullMixBox(
        f,
        endBoxes,
        feeAmount,
        inputBoxes,
        ...
        Seq(distrBurnToken),
        additionalDlogSecrets,
        DHT(WalletHelper.g, gX, gY, gXY, nextY) +: additionalDHTuples
      )
      val expectedFee = ErgoLikeTransaction.serializer
        .toBytes(tx.asInstanceOf[SignedTransactionImpl].getTx)
        .length * MainConfigs.dynamicFeeRate
      if (expectedFee != feeAmount && dynamicFee) {
        return spendFullMixBoxNextBob(
          f,
          h,
          nextY,
          expectedFee,
  ...
  trait HalfMixBoxSpender {
    ...
    def spendHalfMixBox(
      ...
      additionalDHTuples: Array[DHT],
      numToken: Int
    )(implicit ctx: BlockchainContext): (FullMixTx, Boolean) =
      spendHalfMixBox(
        halfMixBox,
        ctx.getBoxesById(inputBoxIds: _*),
        fee,
        changeAddress,
        additionalDlogSecrets,
        additionalDHTuples,
  ...
  trait HalfMixBoxCreator {
    ...
    def createHalfMixBox(
      ...
      mixingTokenId: String,
      mixingTokenAmount: Long
    )(implicit ctx: BlockchainContext): HalfMixTx =
      createHalfMixBox(
        ctx.getBoxesById(inputBoxIds: _*),
        feeAmount,
        changeAddress,
