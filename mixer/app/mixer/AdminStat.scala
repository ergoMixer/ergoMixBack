...
import dao.{AllAdminDAO, CommissionIncomeDAO, DAOUtils, IncomeStateDAO, TokenIncomeDAO}
...
@Singleton
class AdminStat @Inject()(networkUtils: NetworkUtils,
  ...
  def scanEntryTxs(): Unit = {
    ...
    val needToProcess = if ((total - processed) >= AdminConfigs.maxTxToStat) AdminConfigs.maxTxToStat else total - processed
    ...
    invalidStats.foreach(stat => {
      ...
      if (tx.isDefined) {
        try {
          ...
          case a: Throwable =>
            daoUtils.awaitResult(incomeStateDAO.updateRetryNum(IncomeState(stat.orderNum, stat.txId, stat.retryNum + 1)))
            logger.error(s"Admin: An error occurred while processing tx ${tx.get.id} for ${stat.retryNum + 1} time. Stacktrace below")
            logger.error(ergoMixerUtils.getStackTraceStr(a))
        }
      }
    })
  ...
  def processTransaction(tx: SpendTx, orderNum: Int, retried: Boolean = false): Unit = {
    ...
    if (tx.outboxes.head.address == networkUtils.tokenErgoMix.get.halfMixAddress.toString) {
      processedObj = Option(processIncomeBoxes(tx.timestamp, tx.inboxes(1), tx.outboxes(2), tx.outboxes(1), tx.outboxes.head))
      ...
      processedObj = Option(processIncomeBoxes(tx.timestamp, tx.inboxes(2), tx.outboxes(3), tx.outboxes(2), tx.outboxes.head))
      logger.info(s"processed tx ${tx.id} with order num $orderNum")
    }
  ...
  def processIncomeBoxes(
    ...
    val tokenIncomeObj = Some(TokenIncome(ergoMixerUtils.getBegOfDayTimestamp(timestamp), tokenBatch.toInt, 1, tokenBatchPrice))
    val ergCommissionIncomeObj = Some(CommissionIncome(timestamp, "", ergRing, 1, ergCommission, ergExcess))
    val tokenCommissionIncomeObj = if (mixingTokenId.nonEmpty)
      Some(CommissionIncome(timestamp, mixingTokenId, tokenRing, 1, tokenCommission, tokenExcess)) else Option.empty
