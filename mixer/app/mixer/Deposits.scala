      ...
      if (allBoxes.isEmpty && knownIds.isEmpty && timeValidate) {
        ...
        allBoxes.filterNot(box => knownIds.contains(box.id)).foreach { box =>
          insertDeposit(
            ...
            box.id,
            req.neededAmount,
            box.getToken(req.tokenId),
            req.neededTokenAmount
          )
          logger.info(
            s"Successfully processed deposit of ${box.amount} with boxId ${box.id} for depositAddress ${req.depositAddress}"
          )
        }
      }
    }

  implicit private val insertReason: String = "Deposits.insertDeposit"
