...
import dao.mixing.{DistributeTransactionsDAO, MixingGroupRequestDAO, MixingRequestsDAO}
import dao.DAOUtils
...
class GroupMixer @Inject() (
      ...
      daoUtils.awaitResult(mixingGroupRequestDAO.queued).foreach { req =>
        ...
        if (confirmedErgDeposits > 0 || confirmedTokenDeposits > 0) {
          ...
          if (req.tokenId.isEmpty) logger.info(s"  processed confirmed deposits $confirmedErgDeposits")
            ...
            logger.info(
              s"  processed confirmed deposits, erg: $confirmedErgDeposits, ${req.tokenId}: $confirmedTokenDeposits"
            )
        }
  ...
  def processStartingGroup(req: MixGroupRequest): Unit = networkUtils.usingClient { implicit ctx =>
    ...
    if (daoUtils.awaitResult(distributeTransactionsDAO.existsByMixGroupId(req.id))) {
      ...
      daoUtils
        .awaitResult(distributeTransactionsDAO.selectByMixGroupId(req.id))
        .foreach { tx =>
      ...
      for (i <- transactions.indices) {
        ...
        val new_tx =
          DistributeTx(req.id, tx.getId, i + 1, WalletHelper.now, tx.toJson(false).getBytes("utf-16"), inputs)
        ...
        try
          ...
          case e: Throwable =>
            logger.error(
              s"  transaction got refused by the node! maybe it doesn't support chained transactions, waiting... consider updating your node for a faster mixing experience."
            )
