...
class AdminCharger @Inject() (networkUtils: NetworkUtils, ergoMixerUtils: ErgoMixerUtils) {
  ...
  def handleFeeCharging(implicit ctx: BlockchainContext): Unit =
    try {
      ...
      if (toChargeFees.nonEmpty) {
        ...
        toChargeFees.foreach { fee =>
          ...
          val signed = prover.sign(
            txB
              .addInputs(usingIncome._1 :+ fee: _*)
              .addOutputs(feeCp)
      ...
      } else logger.info(s"there isn't any fee emission boxes to be charge!")
    } catch {
      case a: Throwable =>
        logger.error(s" [handleFeeCharging: An error occurred. Stacktrace below")
  ...
  def handleTokenCharging(implicit ctx: BlockchainContext): Unit =
    try {
      val toChargeTokens = AdminConfigs.tokens
        .filter(token => token.getTokens.get(0).getValue <= AdminConfigs.tokenThreshold)
        .sortBy(token => token.getTokens.get(0).getValue)

      if (toChargeTokens.nonEmpty) {
        logger.info(
          s"${toChargeTokens.length} token boxes need to be charged. Will charge ${Math.min(toChargeTokens.length, AdminConfigs.maxTokenBoxInInput)} of them now."
        )
      ...
      } else logger.info(s"there isn't any token emission boxes to be charge!")
    } catch {
      case a: Throwable =>
        logger.error(s" [handleTokenCharging: An error occurred. Stacktrace below")
        logger.error(ergoMixerUtils.getStackTraceStr(a))
    }
  ...
  def getNeededIncomeBoxes(neededErg: Long): (Seq[InputBox], Long) = {
    ...
    var usingIncome: Seq[InputBox] = Seq[InputBox]()
    toUseIncomes.foreach { box =>
      ergNeed -= box.getValue
      usingIncome :+= box
      if (ergNeed < 0) {
        toUseIncomes = toUseIncomes.drop(usingIncome.length)
        return (usingIncome, ergNeed)
      }
    }
    toUseIncomes = toUseIncomes.drop(usingIncome.length)
    (usingIncome, ergNeed)
