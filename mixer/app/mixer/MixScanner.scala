...
class MixScanner @Inject() (networkUtils: NetworkUtils, explorer: BlockExplorer) {
        ...
        tx.outboxes.flatMap { outbox =>
          outbox.mixBox(networkUtils.tokenErgoMix.get) match {
            case Some(Right(FBox(fullMixBoxId, r4, r5, `gZ`))) if r4.exp(secret) == r5 =>
              ...
              FollowedMix(currentRound, isAlice = true, halfMixBoxId, Some(fullMixBoxId)) +: followFullMix(
                fullMixBoxId,
                currentRound,
                masterSecret
              )
        ...
        tx.outboxes.flatMap { outbox =>
          outbox.mixBox(networkUtils.tokenErgoMix.get) match {
            ...
            case Some(Right(FBox(nextFullMixBoxId, g4, `gZ`, g6))) => // spent as Bob
              ...
              val halfMixBoxId =
                tx.inboxes.find(_.address == networkUtils.tokenErgoMix.get.halfMixAddress.toString).get.id
              FollowedMix(nextRound, isAlice = false, halfMixBoxId, Some(nextFullMixBoxId)) +: followFullMix(
                nextFullMixBoxId,
                nextRound,
                masterSecret
              )
  ...
  def followHopMix(
    ...
    currentRound: Int,
    masterSecret: BigInt
  ): (Seq[FollowedHop], Option[FollowedWithdraw]) = {
