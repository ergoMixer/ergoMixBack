...
class FullMixer @Inject() (
    ...
    fullMixes.foreach {
      case (
        ...
        try {
          ...
          val tokenBoxId = daoUtils
            .awaitResult(tokenBoxIdFuture)
            .getOrElse(throw new Exception(s"mixId $mixId not found in TokenEmissionBox"))
          processFullMix(
            mixId,
            maxRounds,
            withdrawAddress,
  ...
  implicit private val insertReason: String = "FullMixer.processFullMix"
  ...
  private def processFullMix(
    ...
    currentRound: Int,
    halfMixBoxId: String,
    optEmissionBoxId: Option[String],
    tokenBoxId: String,
    ...
    if (fullMixBoxConfirmations >= MainConfigs.numConfirmation) {
      ...
      explorer.getSpendingTxId(fullMixBoxId) match {
        case Some(txId) => // spent
          if (!daoUtils.awaitResult(withdrawDAO.existsByTxId(txId))) { // we need to fast-forward, full box is spent but its not withdraw... by rescanning block-chain
            logger.error(
              s" [FULL:$mixId ($currentRound) ${str(isAlice)}] [ERROR] Rescanning because full:$fullMixBoxId is spent"
            )
        ...
        case None => // not spent, good to go
          ...
          val optHalfMixBoxId = getRandomValidBoxId(
            getHalfBoxes
              ...
              .filter(box =>
                box.amount == fullMixBox.amount && box.getToken(mixingTokenId) == fullMixBox.getToken(mixingTokenId)
          ...
          if (
            numTokens < 2 || (withdrawStatus.equals(WithdrawRequested.value) || withdrawStatus.equals(
              HopRequested.value
            )) || (currentRound >= maxRounds && MainConfigs.stopMixingWhenReachedThreshold)
          ) {
            if (withdrawAddress.nonEmpty) {
              val optFeeEmissionBoxId = getRandomValidBoxId(
                getFeeBoxes.map(_.id).filterNot(id => daoUtils.awaitResult(emissionDAO.existsByBoxId(id)))
              )
              if (withdrawDAO.shouldWithdraw(mixId, fullMixBoxId) && optFeeEmissionBoxId.nonEmpty) {
                val tx = if (withdrawStatus.equals(HopRequested.value)) {
                  ...
                  val tx = aliceOrBob.spendFullMixBox(
                    isAlice,
                    secret,
                    fullMixBoxId,
                    hopAddress,
                    Array[String](optFeeEmissionBoxId.get),
                  ...
                  val new_withdraw =
                    WithdrawTx(mixId, tx.getId, currentTime, fullMixBoxId + "," + optFeeEmissionBoxId.get, txBytes)
                  withdrawDAO.updateById(new_withdraw, HopRequested.value)
                  logger.info(
                    s" [FULL:$mixId ($currentRound) ${str(isAlice)}] Hop txId: ${tx.getId}, is requested: ${withdrawStatus
                        .equals(WithdrawRequested.value) || withdrawStatus.equals(HopRequested.value)}"
                  )
                  tx
                } else {
                  val tx = aliceOrBob.spendFullMixBox(
                  ...
                  logger.info(
                    s" [FULL:$mixId ($currentRound) ${str(isAlice)}] Withdraw txId: ${tx.getId}, is requested: ${withdrawStatus
                        .equals(WithdrawRequested.value) || withdrawStatus.equals(HopRequested.value)}"
                  )
                  tx
                }
              ...
              logger.info(
                s" [FULL:$mixId ($currentRound) ${str(isAlice)}] is done mixing but no withdraw address to withdraw!"
              )
            }
          } else { // need to remix
            ...
            if (optFeeEmissionBoxId.nonEmpty) { // proceed only if there is at least one fee emission box
              ...
              def nextAlice = {
                ...
                halfMixDAO.insertHalfMix(
                  HalfMix(mixId, nextRound, currentTime, halfMixTx.getHalfMixBox.id, isSpent = false)
                )
                ...
                mixTransactionsDAO.updateById(
                  MixTransaction(halfMixTx.getHalfMixBox.id, new_tx.getId, new_tx.toJson(false).getBytes("utf-16"))
                )
                logger.info(
                  s" [FULL:$mixId ($currentRound) ${str(isAlice)}] --> Alice [full:$fullMixBoxId, fee:$feeEmissionBoxId], txId: ${halfMixTx.tx.getId}"
                )
                try
              ...
              def nextBob(halfMixBoxId: String) = {
                ...
                val (fullMixTx, bit) = aliceOrBob.spendFullMixBox_RemixAsBob(
                  isAlice,
                  secret,
                  fullMixBoxId,
                  nextSecret,
                ...
                mixTransactionsDAO.updateById(
                  MixTransaction(bobFullMixBox.id, new_tx.getId, new_tx.toJson(false).getBytes("utf-16"))
                )
                logger.info(
                  s" [FULL:$mixId ($currentRound) ${str(isAlice)}] --> Bob [full:$fullMixBoxId, half:$halfMixBoxId, fee:$feeEmissionBoxId], txId: ${fullMixTx.tx.getId}"
                )
                try
              ...
              if (optHalfMixBoxId.isEmpty) {
                ...
                if (MainConfigs.mixOnlyAsBob) {
                  logger.info(
                    s" [FULL:$mixId ($currentRound) ${str(isAlice)}] --> Ignored because mixOnlyAsBob is set to true and no half-box is available currently!"
                  )
                  return
                }
      ...
      logger.info(
        s" [FULL:$mixId ($currentRound) ${str(isAlice)}] Insufficient confirmations ($fullMixBoxConfirmations) [full:$fullMixBoxId]"
      )
          ...
          case e: Throwable =>
            ...
            explorer.doesBoxExist(halfMixBoxId) match {
              case Some(false) =>
                ...
                logger.error(
                  s"  [FULL:$mixId ($currentRound) ${str(isAlice)}] [ERROR] Rescanning [half:$halfMixBoxId disappeared]"
                )
              ...
              case Some(true) =>
                if (
                  networkUtils.isDoubleSpent(halfMixBoxId, fullMixBoxId) || (currentRound == 0 && networkUtils
                    .isDoubleSpent(tokenBoxId, fullMixBoxId))
                ) {
                  // the halfMixBox used in the fullMix has been spent, while the fullMixBox generated has zero confirmations.
                  try {
                    logger.info(
                      s"  [FULL:$mixId ($currentRound) ${str(isAlice)}] <-- Bob (undo). [full: $fullMixBoxId not spent while half: $halfMixBoxId or token: $tokenBoxId spent]"
                    )
                  ...
                  optEmissionBoxId.map { emissionBoxId =>
                    if (networkUtils.isDoubleSpent(emissionBoxId, fullMixBoxId)) {
                      ...
                      try {
                        logger.info(
                          s"  [FULL:$mixId ($currentRound) ${str(isAlice)}] <-- Bob (undo). [full:$fullMixBoxId not spent while fee:$emissionBoxId spent]"
                        )
                        allMixDAO.undoMixStep(mixId, currentRound, fullMixBoxId, isFullMix = true)
                      } catch {
                        case a: Throwable =>
