...
class AdminParams @Inject() (networkUtils: NetworkUtils, ergoMixerUtils: ErgoMixerUtils) {
  ...
  def handleParams(): Unit =
    networkUtils.usingClient { implicit ctx =>
      ...
      if (AdminConfigs.tokenLevels.isEmpty)
        AdminConfigs.tokenLevels =
          AdminConfigs.tokens.last.getRegisters.get(0).getValue.asInstanceOf[Coll[(Int, Long)]].toArray.sorted
      ...
      if (AdminConfigs.desiredFee._1 == 0 || AdminConfigs.desiredFee._2 == 0) {
        val dynamicFeeRate =
          if (AdminConfigs.supports.last.getRegisters.size() >= 5)
            AdminConfigs.supports.last.getRegisters.get(4).getValue.asInstanceOf[Long]
          else 1000L
        AdminConfigs.desiredFee =
          (AdminConfigs.fees.last.getRegisters.get(0).getValue.asInstanceOf[Long], dynamicFeeRate)
      }
  ...
  def handleIncomeParams(implicit ctx: BlockchainContext): Unit =
    AdminConfigs.tokens.foreach { tokenBox =>
      if (
        tokenBox.getRegisters
          .get(0)
          .getValue
          .asInstanceOf[Coll[(Int, Long)]]
          .toArray
          .toSeq
          .sorted != AdminConfigs.tokenLevels.sorted ||
        ...
        val signed = prover.sign(
          txB
            .addInputs(tokenBox, feeIn)
  ...
  def handleFeeParams(implicit ctx: BlockchainContext): Unit = {
    ...
    if (
      AdminConfigs.supports.count { box =>
        val dynamicFeeRate =
          if (box.getRegisters.size() >= 5) box.getRegisters.get(4).getValue.asInstanceOf[Long] else 1000L
      ...
      val signed = prover.sign(
        txB
          .addInputs(AdminConfigs.supports :+ feeIn: _*)
          .addOutputs(outs: _*)
          .fee(AdminConfigs.fee)
          .sendChangeTo(TokenErgoMix.mixerIncome)
          .build()
  ...
  def handleAddSupport(implicit ctx: BlockchainContext): Unit = {
    AdminConfigs.toAddSupport =
      AdminConfigs.toAddSupport.filterNot(req => AdminConfigs.supports.exists(EntityInfo(_).equals(req._1)))
    ...
    if (toAddOpt.nonEmpty) {
      val usableFees = toUseFees
        .slice(toUseInd, toUseFees.length)
        .filter(fee => fee.getValue >= AdminConfigs.fee * 2 + AdminConfigs.supportBoxValue)
  ...
  def handleRemoveSupport(implicit ctx: BlockchainContext): Unit = {
    AdminConfigs.supports
      .filter(sup => AdminConfigs.toRemoveSupport.exists(e => e.equals(EntityInfo(sup))))
      .foreach { sup =>
    ...
    AdminConfigs.toRemoveSupport =
      AdminConfigs.toRemoveSupport.filter(e => AdminConfigs.supports.exists(sup => e.equals(EntityInfo(sup))))
  }
}
