...
@Singleton()
class TokenEmissionDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext)
     ...
     * @param boxId String
     */
    def existsByBoxId(boxId: String): Future[Boolean] = db.run(tokenEmissions.filter(box => box.boxId === boxId).exists.result)
