...
class AllMixDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider, daoUtils: DAOUtils, explorer: BlockExplorer)(implicit executionContext: ExecutionContext)
    ...
    def groupRequestsProgress: Future[Seq[(String, Int, String, BigInt, String, String)]] = {
        val query = for {
            ((halfMix, state), request) <- halfMixes.filter(mix => mix.isSpent === false) join
                mixes on((mix, state) => {mix.mixId === state.mixId && mix.round === state.round}) join
        ...
        } yield (halfMix.mixId, state.round, halfMix.halfMixBoxId, request.masterKey, request.withdrawStatus, request.withdrawAddress)
        db.run(query.result)
    }
    ...
    def groupFullMixesProgress: Future[Seq[(String, Int, String, BigInt, Boolean, String, Int, String, String, String)]] = {
        ...
        } yield (fullMix.mixId, request.numRounds, request.withdrawAddress, request.masterKey, state.isAlice,
    ...
    def groupHopMixesProgress: Future[Seq[HopMix]] = {
        ...
        val lastHopMixes = (hopMixes join lastHops on((hopMix, lastHop) => hopMix.mixId === lastHop._1 && hopMix.round === lastHop._2)).map{ case(hopMix, _) => hopMix}
        val query = for {
            (hopMix, _) <- lastHopMixes join mixingRequests.filter(request => request.withdrawStatus === UnderHop.value) on(_.mixId === _.mixId)
        } yield hopMix
        db.run(query.result)
    }
    ...
    def undoMixStep(mixId: String, round: Int, boxId: String, isFullMix: Boolean): Unit = {
        val currRound = daoUtils.awaitResult(db.run(mixes.filter(mix => mix.mixId === mixId).map(_.round).result.headOption)).getOrElse(throw new Exception(s"No entry exists for $mixId in mixStateTable"))
        if (currRound != round) throw new Exception(s"Current round ($currRound) != undo round ($round)")

        val maxRound = daoUtils.awaitResult(db.run(mixHistories.filter(history => history.mixId === mixId).map(_.round).max.result)).getOrElse(throw new Exception(s"No entry exists for $mixId in mixHistoryTable"))
        ...
        if (round == 0) {
            ...
            val prevMixState = daoUtils.awaitResult(db.run(mixHistories.filter(history => history.mixId === mixId && history.round === prevRound).result.headOption)).getOrElse(throw new Exception(s"No history found for previous round $prevRound and mixId $mixId"))
            db.run(DBIO.seq(
                mixes.filter(mix => mix.mixId === mixId).map(mix => (mix.round, mix.isAlice)).update(prevRound, prevMixState.isAlice),
                emissions.filter(box => box.mixId === mixId && box.round === round).delete
            ))
        }
