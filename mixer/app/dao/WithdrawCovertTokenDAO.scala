...
@Singleton()
class WithdrawCovertTokenDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext)
     ...
     *
     */
    def selectRequestedWithdraws: Future[Seq[CovertAssetWithdrawTx]] = db.run(covertTokenTx.filter(token => token.withdrawStatus === CovertAssetWithdrawStatus.Requested.value).result)

    /**
     * selects non-completed withdraws
     *
     */
    def selectNotProcessedWithdraws: Future[Seq[CovertAssetWithdrawTx]] = db.run(covertTokenTx.filter(token => token.withdrawStatus === CovertAssetWithdrawStatus.NoWithdrawYet.value).result)

    /**
     * inserts a request for withdraw
     *
     */
    def insert(assetWithdrawRequest: CovertAssetWithdrawTx): Future[Unit] = db.run(covertTokenTx += assetWithdrawRequest).map(_ => ())
