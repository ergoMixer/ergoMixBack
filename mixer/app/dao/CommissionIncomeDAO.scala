...
trait CommissionIncomeComponent {
    ...
    class CommissionIncomeTable(tag: Tag) extends Table[CommissionIncome](tag, "COMMISSION_INCOME") {
        ...
        def * = (timestamp, tokenId, ring, numEntered, commission, donation) <> (CommissionIncome.tupled, CommissionIncome.unapply)
...
@Singleton()
class CommissionIncomeDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext)
