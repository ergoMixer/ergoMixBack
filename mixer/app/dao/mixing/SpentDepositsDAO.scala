...
trait SpentDepositsComponent {
  ...
  class SpentDepositsTable(tag: Tag) extends Table[SpentDeposit](tag, "SPENT_DEPOSITS") {
    ...
    def * = (
      ...
      spentTime,
      purpose
    ) <> (SpentDeposit.tupled, SpentDeposit.unapply)
  }

  class SpentDepositsArchivedTable(tag: Tag)
    extends Table[(String, String, Long, Long, Long, String, Long, String, String)](tag, "SPENT_DEPOSITS_ARCHIVED") {
    def address = column[String]("ADDRESS")
...
@Singleton()
class SpentDepositsDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(
  ...
  def existsByBoxId(boxId: String): Future[Boolean] =
    db.run(spentDeposits.filter(deposit => deposit.boxId === boxId).exists.result)
   ...
   */
  def insertDeposit(deposit: SpentDeposit)(implicit insertReason: String): Future[Unit] = db.run(
    DBIO.seq(
