...
@Singleton()
class EmissionDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(
   ...
   * @param boxId String
   */
  def insert(mixId: String, round: Int, boxId: String): Future[Unit] =
    db.run(emissions += (mixId, round, boxId)).map(_ => ())
