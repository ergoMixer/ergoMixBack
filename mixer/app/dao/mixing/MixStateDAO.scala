...
@Singleton()
class MixStateDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(
   ...
   * @param mixID String
   */
  def selectByMixId(mixID: String): Future[Option[MixState]] =
    db.run(mixes.filter(state => state.mixId === mixID).result.headOption)
  ...
  def minRoundsByMixId(mixID: String, numRounds: Int): Future[Int] = {
    val query = for {
      ...
      minRounds <- DBIO.successful(
                     Math.min(
                       state.getOrElse(throw new Exception("corresponding mixId not found in MixState table")).round,
                       numRounds
                     )
                   )
    } yield minRounds
   ...
   */
  def updateById(mixState: MixState): Future[Unit] =
    db.run(mixes.filter(mix => mix.mixId === mixState.id).update(mixState)).map(_ => ())
   ...
   * @param mixState MixState
   */
  def updateInRescan(mixState: MixState): Future[Unit] = db.run(
    DBIO.seq(
