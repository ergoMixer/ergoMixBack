...
trait MixingGroupRequestComponent {
  ...
  class MixingGroupRequestTable(tag: Tag) extends Table[MixGroupRequest](tag, "MIXING_GROUP_REQUEST") {
    ...
    def * = (
      ...
      tokenId,
      masterKey
    ) <> (MixGroupRequest.tupled, MixGroupRequest.unapply)
  }

}

@Singleton()
class MixingGroupRequestDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(
   ...
   */
  def queued: Future[Seq[MixGroupRequest]] =
    db.run(groupRequests.filter(req => req.status === GroupMixStatus.Queued.value).result)

  /**
   * selects starting groupRequests
   */
  def starting: Future[Seq[MixGroupRequest]] =
    db.run(groupRequests.filter(req => req.status === GroupMixStatus.Starting.value).result)

  /**
   * selects active groupRequests
   */
  def active: Future[Seq[MixGroupRequest]] =
    db.run(groupRequests.filterNot(req => req.status === GroupMixStatus.Complete.value).result)

  /**
   * selects completed groupRequests
   */
  def completed: Future[Seq[MixGroupRequest]] =
    db.run(groupRequests.filter(req => req.status === GroupMixStatus.Complete.value).result)
