...
@Singleton()
class DistributeTransactionsDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(
   ...
   * @param groupId String
   */
  def selectSpentTransactions(groupId: String): Future[Seq[DistributeTx]] =
    db.run(distributeTransactions.filter(tx => tx.mixGroupId === groupId && tx.order > 0).result)
   ...
   */
  def selectByMixGroupId(mixGroupId: String): Future[Seq[DistributeTx]] =
    db.run(distributeTransactions.filter(tx => tx.mixGroupId === mixGroupId).sortBy(_.order).result)
   ...
   * @param mixGroupId String
   */
  def selectInputsByMixGroupId(mixGroupId: String): Future[Seq[String]] =
    db.run(distributeTransactions.filter(tx => tx.mixGroupId === mixGroupId).map(_.inputs).result)
   ...
   * @param mixGroupId String
   */
  def existsByMixGroupId(mixGroupId: String): Future[Boolean] =
    db.run(distributeTransactions.filter(req => req.mixGroupId === mixGroupId).exists.result)
   ...
   */
  def zeroChainByMixGroupIdAndTime(mixGroupId: String, createdTime: Long): Future[Seq[DistributeTx]] = db.run(
    distributeTransactions.filter(tx => tx.mixGroupId === mixGroupId && tx.order === 0 && tx.time <= createdTime).result
  )
   ...
   * @param groupId String
   */
  def deleteByGroupId(groupId: String): Unit =
    db.run(distributeTransactions.filter(tx => tx.mixGroupId === groupId).delete).map(_ => ())
}
