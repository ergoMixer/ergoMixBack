...
trait HalfMixComponent {
  ...
  class HalfMixArchivedTable(tag: Tag)
    extends Table[(String, Int, Long, String, Boolean, String)](tag, "HALF_MIX_ARCHIVED") {
    def mixId = column[String]("MIX_ID", O.PrimaryKey)
...
@Singleton()
class HalfMixDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(
   ...
   * @param round Int
   */
  def selectOption(mixId: String, round: Int): Future[Option[HalfMix]] =
    db.run(halfMix.filter(mix => mix.mixId === mixId && mix.round === round).result.headOption)
  ...
  def boxIdByMixId(mixId: String): Future[Option[String]] =
    db.run(halfMix.filter(mix => mix.mixId === mixId).map(_.halfMixBoxId).result.headOption)
  ...
  def deleteFutureRounds(mixId: String, round: Int): Future[Unit] =
    db.run(halfMix.filter(mix => mix.mixId === mixId && mix.round > round).delete).map(_ => ())
   ...
   */
  def updateById(new_mix: HalfMix)(implicit insertReason: String): Future[Unit] = db.run(
    DBIO.seq(
   ...
   * @param new_mix HalfMix
   */
  def insertHalfMix(new_mix: HalfMix)(implicit insertReason: String): Future[Unit] = db.run(
    DBIO.seq(
      halfMix += new_mix,
      halfMixArchive += (new_mix.mixId, new_mix.round, new_mix.createdTime, new_mix.halfMixBoxId, new_mix.isSpent, insertReason)
    )
  ...
  def getMixBoxIdByRound(mixId: String, round: Int): Future[Option[String]] =
    db.run(halfMix.filter(mix => mix.mixId === mixId && mix.round === round).map(_.halfMixBoxId).result.headOption)
