...
trait UnspentDepositsComponent {
  ...
  class UnspentDepositsArchivedTable(tag: Tag)
    extends Table[(String, String, Long, Long, Long, String)](tag, "UNSPENT_DEPOSITS_ARCHIVED") {
    def address = column[String]("ADDRESS")
...
@Singleton()
class UnspentDepositsDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(
  ...
  def existsByBoxId(boxId: String): Future[Boolean] =
    db.run(unspentDeposits.filter(deposit => deposit.boxId === boxId).exists.result)
   ...
   * @param address String
   */
  def selectByAddress(address: String): Future[Seq[Deposit]] =
    db.run(unspentDeposits.filter(deposit => deposit.address === address).result)
   ...
   * @param boxId String
   */
  def delete(boxId: String): Future[Unit] =
    db.run(unspentDeposits.filter(deposit => deposit.boxId === boxId).delete).map(_ => ())
   ...
   * @param address String
   */
  def deleteByAddress(address: String): Future[Unit] = db.run(
    DBIO.seq(
   ...
   */
  def insertDeposit(deposit: Deposit)(implicit insertReason: String): Future[Unit] = db.run(
    DBIO.seq(
