...
@Singleton()
class MixingRequestsDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext)
    extends MixingRequestsComponent with MixStateComponent
     ...
     */
    def selectAllByMixAndWithdrawStatus(groupId: String, mixStatus: MixStatus, withdrawStatus: String): Future[Seq[MixRequest]] = {
        val query = for {
            req <- mixingRequests if req.groupId === groupId && req.mixStatus === mixStatus && req.withdrawStatus === withdrawStatus
        } yield req.mixRequest
        db.run(query.result)
    }
     ...
     *
     */
    def selectAllQueued: Future[Seq[MixingRequest]] = db.run(mixingRequests.filter(req => req.mixStatus === MixStatus.fromString(Queued.value) && req.depositCompleted === true).result)
     ...
     * @param address String
     */
    def deleteByWithdrawAddress(address: String): Future[Unit] = db.run(mixingRequests.filter(req => req.withdrawAddress === address).delete).map(_ => ())
     ...
     * @param groupId String
     */
    def deleteByGroupId(groupId: String): Future[Unit] = db.run(mixingRequests.filter(req => req.groupId === groupId).delete).map(_ => ())

    /**
    * returns non-completed deposits
    *
    */
    def nonCompletedDeposits: Future[Seq[MixingRequest]] = db.run(mixingRequests.filter(req => req.depositCompleted === false).result)
    ...
    * @param address String
    */
    def existsByDepositAddress(address: String): Future[Boolean] = db.run(mixingRequests.filter(req => req.depositAddress === address).exists.result)
