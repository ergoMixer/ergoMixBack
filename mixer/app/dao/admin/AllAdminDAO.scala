...
@Singleton()
class AllAdminDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(
  ...
  def updateStats(
    ...
    val tokenIncomeQuery = for {
      incomeQ <-
        DBIO.successful(
          tokens.filter(obj => obj.timestamp === tokenIncome.timestamp && obj.mixingLevel === tokenIncome.mixingLevel)
        )
      ...
      _ <- if (incomeOption.isEmpty)
             ...
             incomeQ
               .map(obj => (obj.numEntered, obj.amount))
               .update(
                 (
    ...
    val ergCommisionIncomeQuery = for {
      ...
      _ <- if (commissionOption.isEmpty)
             ...
             commissionQ
               .map(obj => (obj.commission, obj.donation, obj.numEntered))
               .update(
                 (
    ...
    val incomeStateQuery = for {
      ...
      _ <- if (stateOption.isEmpty)
             ...
             stateQ
               .map(obj => (obj.orderNum, obj.retryNum))
               .update(
                 (
                   state.orderNum,
                   state.retryNum
    ...
    if (tokenCommissionIncomeOption.isDefined) {
      ...
      db.run(
        DBIO.seq(tokenIncomeQuery, ergCommisionIncomeQuery, tokenCommisionIncomeQuery, incomeStateQuery).transactionally
      )
    } else {
