...
ErgoMixer è un'applicazione web per la miscelazione di erg e token basata sulla piattaforma Ergo. ErgoMixer è completamente serverless; Ha solo bisogno di connettersi a explorer e un nodo (qualsiasi nodo! non è necessario api_key). Per ulteriori informazioni su come funziona, vedere [qui](https://github.com/ergoMixer/ergoMixBack/wiki/How-it-works-ergoMixer).
...
Quando si apre il Mixer, la home page mostra le informazioni sul sistema e sul funzionamento di ogni componente. ErgoMixer sarà disponibile anche dalla barra dei menu.
